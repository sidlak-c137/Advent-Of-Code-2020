...
// toString for coords
string getCoord(int x, int y, int z, int w);
...
int getNeighbors(const unordered_set<vector<int>*>& v, string c, const unordered_map<string, char>& pos) {
    ...
    int count = 0;
    for (auto vec: v) {
        auto it = pos.find(getCoord(x + (*vec)[0], y + (*vec)[1], z + (*vec)[2], w + (*vec)[3]));
        if (it != pos.end()) {
            if (it->second == '#') {
                count++;
            }
...
int cycle(const unordered_set<vector<int>*>& v, int n, unordered_map<string, char>& pos) {
    for (int i = 0; i < n; i++) {
        ...
        for (auto p: pos) {
            ...
            for (auto vec: v) {
                string coord = getCoord(x + (*vec)[0], y + (*vec)[1], z + (*vec)[2], w + (*vec)[3]);
