...
// Tests string
bool test(string& s, unordered_map<int, vector<opt*>*>& v, vector<int>& index, int rule, int depth);
...
int main() {
    ...
    // Add extra rules
    v[8]->push_back(new opt("42 8"));
    v[11]->push_back(new opt("42 11 31"));
    // Count rules
    int count = 0;
    while (std::getline(input, s)) {
        vector<int> index{0};
        if (test(s, v, index, 0, 0)) {
            for (auto i: index) {
                if (i == s.size()) {
                    count++;
                    break;
...
bool test(string& s, unordered_map<int, vector<opt*>*>& v, vector<int>& index, int rule, int depth) {
    ...
    for (auto i: *(v[rule])) {
        ...
        bool res = true;
        if (i->a != -1) {
            res &= test(s, v, temp, i->a, depth);
        }
        if (i->b != -1) {
            res &= test(s, v, temp, i->b, depth);
        }
        if (i->c != -1) {
            res &= test(s, v, temp, i->c, depth);
        }
        if (res) {
            for (auto j: temp) {
                rv.push_back(j);
