...
// Simulates game
bool play(deque<int>& p1, deque<int>& p2);
...
bool play(deque<int>& p1, deque<int>& p2) {
    // Seen board positions
    unordered_set<string> done;
    while (!p1.empty() && !p2.empty()) {
        // Check if board position already exists
        string s;
        for (auto x: p1) {
            s += to_string(x);
        }
        ...
        if (a <= p1.size() && b <= p2.size()) {
            ...
            deque<int> p2new;
            for (int i = 0; i < a; i++) {
                p1new.push_back(p1[i]);
            }
            for (int i = 0; i < b; i++) {
                p2new.push_back(p2[i]);
            }
            // Go to sub game
            p1win = play(p1new, p2new);
        }
