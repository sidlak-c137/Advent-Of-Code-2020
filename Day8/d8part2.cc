...
// returns value of ACC after looping ends (after one inst is modified)
...
void printACC(const vector<instr*>& in) {
    ...
    while (1) {
        ...
        while (index.find(curr) == index.end()) {
            // stop if looping ends
            if (curr >= in.size() || curr < 0) {
                check = true;
                break;
            }
            index.insert(curr);
        ...
        // If looping has ended, print acc
        if (check) {
            cout << "Acc: " << acc << endl;
            break;
        }
